...
- 지수부는 부호를 나타내기 위해 실제 지수에 $2^{10}$ 를 더해서 저장한다.
  - 그렇게 하면 $-$ 의 경우 맨 앞의 bit 는 $0$ 이 되고, $+$ 의 경우 맨 앞의 bit 는 $1$ 이 된다
- 가수부는 52 bit 이므로, 앞에서부터 52개의 2진수를 받으며, 부족할 경우 뒤에 $0$을 채운다 <br> <br>
...
그러므로 지수부는 $(6+2^{10})$ $_{(10)}$ $=$ $10000110_{(2)}$ 이고, 가수부는 $1011110001100110011001100...$ (52자리) 이다
