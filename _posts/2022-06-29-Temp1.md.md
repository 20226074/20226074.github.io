...
float 의 경우, 1 bit 는 $0(+)$  $or$  $1(-)$ 를 나타내고, 11 bit 는 지수부, 52 bit 는 가수부로 사용하여 부동 소수점 표현으로 나타낸다 <br>

### 부동 소수점 표현  

(1) float 로 입력을 받은 것의 정수부와 소수부를 2진수로 나타낸다
- 정수부는 int 와 동일한 논리이고, 소수부는 $2^{-1}, 2^{-2}, ...$ 를 사용하여 근사화시킨다 <br>

(2) 위의 과정으로 만들어진 2진수의 소수점을 이동시켜서 $1.xxx..._{(2)}$ $\times$ $2^E$ 형태로 만들어준다
...
$111.1$ 의 경우, 부호부는 $0(+)$ 이고, 정수부와 소수부를 2진수로 나타내면 $1101111.00011001100...$ 이다 <br>
소수점을 이동시키면 $1.10111100011001100... \times 2^6$ 으로 나타내진다 <br>
그러므로 지수부는 $6+2^{10}$ $=$ $10000110$ $_{(2)}$ 이고, 가수부는 $10111100011001100...$ (52자리) 이다
